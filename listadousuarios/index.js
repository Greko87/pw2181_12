...
function botonDetalle(){
	...
	require('electron').remote.getGlobal('infoUsuarios').nombre=usuarios[this.id].nombre; //para que se vaya al main.js
	...
	pantallaDetalle.loadURL(url.format({
		pathame: path.join(__dirname,'detalleUsuarios.html'),//pasar los datos
		protocol: 'file',
		slashes: true
	}));
